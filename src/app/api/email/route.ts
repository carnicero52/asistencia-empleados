...
// El dominio onboarding@resend.dev SOLO puede enviar emails a tu correo verificado
// Para enviar a otros, verifica un dominio en Resend Dashboard
...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailData = {
      from: `${nombreEmpresa || 'Empresa'} <onboarding@resend.dev>`,
      to: email,
      subject: `Tu Código QR de Acceso - ${nombreEmpresa || 'Empresa'}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #059669 0%, #10b981 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
            <h1 style="margin: 0; font-size: 24px;">${nombreEmpresa || 'Empresa'}</h1>
          ...
          <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
            ...
            <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
              ...
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                  ...
                  <td style="padding: 8px 0; color: #1f2937; border-bottom: 1px solid #e5e7eb;">${(turno || 'diurno').toUpperCase()}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; color: #6b7280; border-bottom: 1px solid #e5e7eb;">Horario:</td>
                  <td style="padding: 8px 0; color: #1f2937; border-bottom: 1px solid #e5e7eb;">${horaEntrada || '08:00'} - ${horaSalida || '17:00'}</td>
                </tr>
              </table>
            </div>
            ...
            <p style="color: #9ca3af; font-size: 12px; text-align: center; margin-top: 30px;">
              Este es un email automático del Sistema de Control de Asistencia de ${nombreEmpresa || 'Empresa'}
            </p>
          </div>
        </div>
      `
    ...
    if (response.ok) {
      ...
      // Manejar error específico de destinatario no verificado
      if (result.message?.includes('not verified') || result.message?.includes('domain')) {
        return NextResponse.json({
          error: '⚠️ Limitación de Resend: Solo puedes enviar emails a tu propia dirección de correo verificada. Para enviar a otros empleados, ve a Resend Dashboard y verifica un dominio.',
